...
//Grid parameters
float sepCo = 1.0f; //separation coeff
float cohCo = 1.0f; //alignement coeff
float aliCo = 1.0f; //alignement coeff
float rr = -1.0f; //Reverse rate
float rein = 0.1f; //a variable used to moderate the velocity
...
public void draw() {
  ...
  if (paintgunGr.isOpen() && bas==1 ) {
    ...
    fill(gunR, gunG, gunB);
    noStroke();
    ellipse(paintgunGr.getPosition()[0]+12*wpc, paintgunGr.getPosition()[1]+29*hpc, pelletSize*0.0125f*height, pelletSize*0.0125f*height);
  }
...
public void gui() {
  ...
  wpc = Math.round(0.01f*width);
  hpc = Math.round(0.01f*height);
  ...
  pelletX = guiX + 15*hpc;
  pelletY = Math.round(gunY-1.52f*2*hpc);
  ...
  reinS = cp5.addSlider("rein", 0, 1, 0.1f, wpc, hpc, 16*wpc, 2*hpc).setCaptionLabel(" Speed").setColor(grey).moveTo(gridGr); //speed slider
  ...
  cohCoS = cp5.addSlider("cohCo", 0.5f, 1.5f, 1, wpc, hpc, 16*wpc, 2*hpc).setCaptionLabel(" Cohesion").moveTo(flockingGr);
  aliCoS = cp5.addSlider("aliCo", 0.5f, 1.5f, 1, wpc, 4*hpc, 16*wpc, 2*hpc).setCaptionLabel(" Alignment").moveTo(flockingGr);
  sepCoS = cp5.addSlider("sepCo", 0.5f, 1.5f, 1, wpc, 7*hpc, 16*wpc, 2*hpc).setCaptionLabel(" Separation").moveTo(flockingGr);
...
public void controlEvent(ControlEvent theEvent) {
  ...
  if (theEvent.isGroup()) {
    ...
    //launch presets
      ...
      activeLayer = PApplet.parseInt(choice);
    ...
    //set new R limits
      ...
      lowLimR = PApplet.parseInt(theEvent.getController().getArrayValue(0));
      uppLimR = PApplet.parseInt(theEvent.getController().getArrayValue(1));
    }
    //set new G limits
    else if (theEvent.isFrom(limitsG)) {
      lowLimG = PApplet.parseInt(theEvent.getController().getArrayValue(0));
      uppLimG = PApplet.parseInt(theEvent.getController().getArrayValue(1));
    }
    //set new B limits
    else if (theEvent.isFrom(limitsB)) {
      lowLimB = PApplet.parseInt(theEvent.getController().getArrayValue(0));
      uppLimB = PApplet.parseInt(theEvent.getController().getArrayValue(1));
    } else if (theEvent.isFrom(stripeOS)) {
      stripeOffset = PApplet.parseInt(theEvent.getController().getArrayValue(0));
      sOff2 = PApplet.parseInt(theEvent.getController().getArrayValue(1));
    }
...
class Grid {
  ...
  int gridX = Math.round(width*0.26f);
  int gridY = Math.round(height*0.07f);
...
class Cell {
  ...
  public void paint() {
    ...
    float clickY = 0;
    if (fs%2==0) {
      float w = PApplet.parseFloat(width);
      float h = PApplet.parseFloat(height);
...
public void aurora() {
  ...
  gunBS.setValue(80);
  res.activate("256 x 160 (Default)");
  n1 = n2 = n3 = n7 = n8 = n9 = 1.0f;
  n4 = n6 = 0.0f;
  neighbox.deactivate("n4").deactivate("n6");
  f1 = f2 = f3 = f4 = 0;
  symmetry = 0;
  rrS.setValue(-0.12f);
  aliCoS.setValue(1.3f);
  cohCoS.setValue(1);
  sepCoS.setValue(1);
  grid.restart(1);
}

public void rorschach() {
  limitsR.setLowValue(0).setHighValue(255);
  limitsG.setLowValue(0).setHighValue(255);
  limitsB.setLowValue(0).setHighValue(255);
  pelletSizeS.setValue(3);
...
public void stripes(int i, int w, int gap, int off, int off2) {
